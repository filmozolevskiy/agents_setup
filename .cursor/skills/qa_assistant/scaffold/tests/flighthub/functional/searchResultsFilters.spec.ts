...
test.describe('flighthub search-results page — filter sidebar / sort / pagination / per-card content', () => {
    ...
    test(
        ...
        async ({ flighthubSearchResultsPage }) => {
            await test.step('THEN the carrier logo and name are rendered', async () => {
                await expect(
                    flighthubSearchResultsPage.resultCardCarrierLogo(0)
                ).toBeVisible();
                await expect(
                    flighthubSearchResultsPage.resultCardCarrierName(0)
                ).toBeVisible();
            });

            await test.step('AND the IATA pair, time block and stops label are rendered', async () => {
                await expect(
                    flighthubSearchResultsPage.resultCardAirportPair(0)
                ).toBeVisible();
                await expect(
                    flighthubSearchResultsPage.resultCardTimeBlock(0)
                ).toContainText(/\d{1,2}:\d{2}\s?(AM|PM)/);
                await expect(
                    flighthubSearchResultsPage.resultCardStopsLabel(0)
                ).toContainText(/Nonstop|Stop|Stops/);
            });

            await test.step('AND the duration and price are rendered', async () => {
                await expect(
                    flighthubSearchResultsPage.resultCardDuration(0)
                ).toContainText(/\d+h\s+\d+m/);
                // Tighten beyond a hardcoded `/CAD\s*\d+/` (which silently
                // missed any prod / non-CA-GeoIP run that defaults to USD)
                // to require ANY currency token from `SupportedCurrency`
                // or its rendered glyph. Keeps this test currency-agnostic
            ...
            await test.step('AND the per-card Select and Show-flight-details buttons resolve uniquely on the first card', async () => {
                await expect(
                    flighthubSearchResultsPage.resultCardSelectButton(0)
                ).toBeVisible();
                await expect(
                    flighthubSearchResultsPage.resultCardShowDetailsButton(0)
                ).toBeVisible();
            });
        }
    );
    ...
    test(
        ...
        async ({ flighthubSearchResultsPage }) => {
            const totalBefore =
                await flighthubSearchResultsPage.readResultsTotal();
            ...
            await test.step('WHEN the user toggles the Nonstop checkbox in the Stops filter', async () => {
                await flighthubSearchResultsPage.toggleStopsFilter(
            ...
            await test.step('THEN the Stops chip appears in the active-filters strip', async () => {
                await expect(
                    flighthubSearchResultsPage.activeFilterChip('stops')
                ).toBeVisible();
            });
            ...
            await test.step('AND the visible result count drops below the original total', async () => {
                await expect
                    .poll(async () => {
                        const visible =
                            await flighthubSearchResultsPage.readResultsVisible();
            ...
            await test.step('WHEN the user clicks Clear all', async () => {
                await flighthubSearchResultsPage.clearAllFilters();
            });
            ...
            await test.step('THEN the active-filters chip is gone and the counter shows the original total again', async () => {
                ...
                await expect
                    .poll(() => flighthubSearchResultsPage.readResultsTotal())
                    .toBe(totalBefore);
            });
        }
    );
    ...
    test(
        ...
        async ({ flighthubSearchResultsPage }) => {
            await test.step('WHEN the user expands the Price filter section', async () => {
                await flighthubSearchResultsPage.toggleFilterSection(
            ...
            await test.step('THEN both price slider thumbs are mounted', async () => {
                await expect(
                    flighthubSearchResultsPage.priceSliderInput('min')
                ).toBeAttached();
                await expect(
                    flighthubSearchResultsPage.priceSliderInput('max')
                ).toBeAttached();
            });
            ...
            await test.step('THEN the Departure / Arrival tabs and both slider thumbs are present', async () => {
                await expect(
                    flighthubSearchResultsPage.flightTimesTab(
                ...
                await expect(
                    flighthubSearchResultsPage.flightTimesSliderInput('min')
                ).toBeAttached();
                await expect(
                    flighthubSearchResultsPage.flightTimesSliderInput('max')
                ).toBeAttached();
            });
        }
    );
    ...
    test(
        ...
        async ({ flighthubSearchResultsPage }) => {
            ...
            await test.step('THEN the nearby + same-airport toggles and the YUL / JFK rows are mounted', async () => {
                await expect(
                    flighthubSearchResultsPage.nearbyAirportsCheckbox
                ).toBeAttached();
                await expect(
                    flighthubSearchResultsPage.sameAirportCheckbox
                ).toBeAttached();
                await expect(
                    flighthubSearchResultsPage.airportFilterCheckbox('YUL')
                ).toBeAttached();
                await expect(
                    flighthubSearchResultsPage.airportFilterCheckbox('JFK')
                ).toBeAttached();
            });
        }
    );
    ...
    test(
        ...
        { tag: '@regression' },
        async ({ flighthubSearchResultsPage }) => {
            await flighthubSearchResultsPage.selectSort(
                Messages.SEARCH_RESULTS_SORT_BEST
            );
            const bestPriceText =
                (await flighthubSearchResultsPage
                    .resultCardPrice(0)
                    .textContent()) ?? '';
    ...
    test(
        ...
        async ({ flighthubSearchResultsPage }) => {
            const totalBefore =
                await flighthubSearchResultsPage.readResultsTotal();
            const cardsBefore =
                await flighthubSearchResultsPage.resultCards.count();

            await test.step('WHEN the user clicks "Load more"', async () => {
                const loaded = await flighthubSearchResultsPage.loadMore();
                expect(loaded).toBeTruthy();
            });

            await test.step('THEN more cards render and the underlying total is unchanged', async () => {
                await expect
                    .poll(() => flighthubSearchResultsPage.resultCards.count())
...
/**
 * Parses a storefront price string ("CAD 223.95", "CAD 1,234.56") to a
