...
/**
 * Per-traveller locator bundle returned by {@link FlighthubBookingConfirmationPage.traveller}.
 *
 ...
 * the same name (see {@link FlighthubBookingConfirmationPage.eTicketRow}).
 */
export interface FlighthubBookingTravellerLocators {
    label: Locator;
    fullName: Locator;
    dateOfBirth: Locator;
    gender: Locator;
...
/**
 * Per-leg locator bundle returned by {@link FlighthubBookingConfirmationPage.itinerarySlice}.
 *
 ...
 * IATA token in DOM order. Carrier name, flight number, cabin, and
 * per-segment duration are intentionally NOT exposed as named locators:
 * they repeat once per segment and overlap with global strings
 * (`Departure`, `BUSINESS RESTRICTED`, ...) that would force fragile
 * `.nth()` indexing. Tests that need them should query the underlying
 * `card` locator directly.
 */
export interface FlighthubBookingSliceLocators {
    card: Locator;
    sliceLabel: Locator;
...
/**
 * Page object for two distinct post-checkout surfaces:
 *
 ...
 *      `seededBookingIdHash()` (see `helpers/shared/seededBooking.ts`).
 *
 ...
 *      `tests/flighthub/e2e/checkoutFlow.spec.ts` (booking-id capture for
 *      Respro cleanup) and by
 *      `tests/flighthub/functional/bookingConfirmationDetail.spec.ts`
 ...
 */
export class FlighthubBookingConfirmationPage {
    ...
    // Legacy alias for the first iteration of this page object — kept for
    // the @destructive E2E that asserts the post-Confirm-and-Book banner.
    ...
    // Issued vs cancelled bookings expose different subsets of the quick-action
    // strip — gate visibility assertions on booking status before calling.
    ...
    get cancellationBanner(): Locator {
        // The storefront renders this banner with a straight apostrophe
        // in `haven't` while the playwright-cli snapshot rewrites it to
        // a curly apostrophe (and vice-versa across browsers). Match a
        // stable prefix that does not depend on the apostrophe so the
        // locator is resilient to that drift.
        return this.page.getByText(
    ...
    // 0-indexed (outbound = 0, return = 1). Slice card has no semantic role,
    // so locators are scoped via an ancestor walk from the stable
    // `Airline confirmation: ...` label at the top of each card.
    itinerarySlice(sliceIdx: number): FlighthubBookingSliceLocators {
        ...
        // (verified 2026-05-08 via a depth-probe on staging2).
        ...
        return {
            ...
            sliceLabel: card
                .getByText(
                    new RegExp(
                        `^(${Messages.SEARCH_RESULTS_FLIGHT_TIMES_DEPARTURE_TAB}|${Messages.BOOKING_ITINERARY_SLICE_RETURN})$`
                    )
                )
    ...
    // 1-indexed to mirror the storefront `Passenger 1` / `Passenger 2` labels.
    // Row has no semantic role; locators are scoped to the closest ancestor
    // <li> that hosts the `Passenger N` label.
    traveller(idx: number): FlighthubBookingTravellerLocators {
     ...
     * source the hash from `seededBookingIdHash()` rather than hard-coding.
     *
    ...
    /**
     * Reads the `Final Total Price` line and returns the trailing currency
     * + amount as displayed (e.g. `'CAD 1,451.42'`). Useful for asserting
     * the cart total survives the redirect from checkout.
     *
