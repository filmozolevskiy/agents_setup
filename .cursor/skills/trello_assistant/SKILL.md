...
5. **`### ⊙ Acceptance criteria`** *(mandatory, always present)* — a checklist of tickable items that defines "done" for this card. The trello_assistant skill always ships this section as a single empty placeholder line (`- [ ] _TO BE DONE_`); the `qa_strategy` skill or the QA team fills it in later. Always a checklist (`- [ ]` items) — prose acceptance criteria are not allowed; the populating writer must conform to the checklist shape.
6. **`### ⊙ Credentials / access`** *(optional — new-integration cards only)* — labeled placeholder the card owner fills by hand. Not on bug / error cards — put any access note in Details there. The agent never fabricates office codes / PCCs / logins.
7. **`### ⊙ QA notes`** *(optional)* — only when there is a shipped fix to verify; omit by default. Staging repro + post-deploy signal, observable by a human or a log query.
8. **`### ⊙ Similar / relevant cards`** *(optional)* — cards that **directly inform** this one (same code path, same supplier behavior, same root cause, or known overlap of fix). One `[title](shortUrl) — short note on the overlap` per line. Drop neighbouring-area cards that only share the supplier. Omit when nothing relevant.
...
2. **Four mandatory `⊙` sections** — `### ⊙ **Short description**` + `### ⊙ **Details**` + `### ⊙ **Visibility**` + `### ⊙ **Acceptance criteria**`, as H3 headings with a blank line after each. **Order:** Short → Details → (optional Possible solution / expected behavior, when present) → Visibility → Acceptance criteria → remaining optional sections. `Acceptance criteria` always ships, even when empty — the placeholder is the single line `- [ ] _TO BE DONE_` and the `qa_strategy` skill or the QA team populates it later (always as `- [ ]` checklist items, never prose). Optional sections (`Possible solution / expected behavior`, `Credentials / access`, `QA notes`, `Similar / relevant cards`) only when they apply — never empty stubs. No other `⊙` blocks; fold extra investigation / repro prose into Details. Migrate legacy cards on edit unless told otherwise.
...
- Do not include optional sections (`Possible solution / expected behavior`, `Credentials / access`, `QA notes`, `Similar / relevant cards`) as empty stubs. Omit a section that has nothing real to say. The four mandatory sections always ship — `Acceptance criteria` is the one allowed exception: it always ships, and starts empty with the placeholder line `- [ ] _TO BE DONE_` for the `qa_strategy` skill / QA team to fill in.
