...
- **No unbounded scans.** Never run `find {}` on `debug_logs` / `optimizer_logs` (rotated window, expensive); never run `SELECT *` without a filter + `LIMIT` against `bookability_*` or `jupiter_booking_errors_v2`. Indexed filters or a tight `date_added` / `created_at` window are mandatory. The same rule lives in `../table_analysis/references/mongodb_query_mechanics.md` for the Mongo CLI specifically.
