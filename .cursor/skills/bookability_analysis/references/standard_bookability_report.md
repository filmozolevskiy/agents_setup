...
For any cluster whose CH `classification_category = 'PAYMENT_ERRORS'`, pull Mongo before deciding whether the cluster is really a payment failure. CH can misclassify post-Sale fare-increase reversals (`loss-limit-fare-increase` → `DeferredRefundPaidStatementItemsAction::run` → `CancelVirtualCardPipe`) as `payment_error` because the wrapper message looks payment-shaped. The canonical example is the Downtowntravel `Virtual card merchant fare statement items failed` cluster, but the same shape can appear under `Credit Card payment declined` and other payment-shaped wrappers.

**The same cleanup chain also fires after a genuine card decline** — every failed `Sale` triggers `DeferredRefundPaidStatementItemsAction::run` and `CancelVirtualCardPipe` to tear down the virtual card, and CH labels that cleanup with the same wrapper text. Reclassify only when **both**: (1) the Mongo trace shows a successful Payhub `Sale` (`"success": true` in the `response` body), **and** (2) `loss-limit-fare-increase` fires **after** that successful Sale. If every Sale on the cluster's transactions failed, CH's `PAYMENT_ERRORS` classification is correct — leave it. The disambiguation step (success-flag aggregation + per-transaction event timeline) and the recorded misjudgement that motivates this rule live in [`known_pitfalls.md`](known_pitfalls.md#downtowntravel--virtual-card-merchant-fare-statement-items-failed-cluster). Read it before flagging any reclassification.

When both conditions hold, ship the cluster as a `CRITICAL` row in the failure-causes table with the reclassification consequence ("drops the bookability rate from X to Y") in `Supplier verbatim`.
