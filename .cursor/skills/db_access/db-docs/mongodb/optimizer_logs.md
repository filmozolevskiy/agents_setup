...
| NDCONE (Condor / DE) | `Ndcone[<ACCOUNT>] shopping/flight`                                                                                                       | Raw Condor NDC 21.3 `shopping/flight` call + response for one leg. `<ACCOUNT>` is the pricing account (`NDCONEDECAD`, `NDCONEDEUSD`, `NDCONEDEEUR`). Endpoint `https://api.ndc.condor.com/de/21_3/shopping/flight`. Confirmed 2026-08-11. Condor error `1000` / `NATIVE ParseError [1,1] Content is not allowed in prolog` seen 100% on this context (see debugger content-sources note). | Top-level `Request`, `Response` (JSON strings — **readable, not placeholders**), `data` (`Operation` / `Url` / `Response time`), `backtrace`. Parse `Response` with `json.loads`; Condor `error[]` + `payloadAttributes.trxID`. |
| NDCONE (Condor / DE) | `ndcone Reprice failure`                                                                                                                  | Wrapper emitted alongside the failing `shopping/flight` call when the reprice yields nothing usable (same shape as the pkfare / travelport equivalents). Confirmed 2026-08-11.                                                       | Top-level `exception`.                                                                                                                                                                                    |
