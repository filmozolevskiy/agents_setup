...
| Travelport Plus NDC  | `travelportplusndc::Reprice-original-operands`                                                                                            | The operand(s) we asked Travelport Plus NDC to reprice. One document per leg; `_scopes` carries `Reprice[master_N]`. Confirmed 2026-07-09.                                                                                          | Top-level `operands` (JSON string; parse with `json.loads`) + `dd_trace_id` / `dd_span_id` / `dd_env`.                                                                                                     |
| Travelport Plus NDC  | `travelport-plus-ndc-api[<ACCOUNT>NDC] /11/air/catalog/search/catalogproductofferings`                                                    | Raw Travelport NDC `CatalogProductOfferings` search call + response for one leg. `<ACCOUNT>` is the pricing account (e.g. `23FQ`, `2G3B`). Confirmed 2026-07-09. Price split lives at `CatalogProductOfferings[].ProductBrandOptions[].ProductBrandOffering[].BestCombinablePrice` (`Base`, `TotalTaxes`, `TotalPrice`); for tax-heavy AC fares `Base=0.00` and all cost is in `TotalTaxes`. | Top-level `Request`, `Response` (JSON strings), `Operation`, `Response time`, `backtrace`. Parse `Response` with `json.loads`.                                                                             |
| Travelport Plus NDC  | `travelportplusndc-no-matching-itineraries` / `travelportplusndc Reprice failure`                                                         | Travelport returned something but the Optimizer could not reconcile it to the anchor itinerary (fires as a pair, same shape as the pkfare equivalents).                                                                              | Top-level `itinerary` / `mismatchingItineraries` (no-matching) and `exception` (failure).                                                                                                                 |
| NDCONE (Condor / DE) | `Ndcone::Reprice-original-operands`                                                                                                       | The operand(s) we asked NDCONE (Condor) to reprice. One document per leg; `_scopes` carries `Reprice[master_N]`. Confirmed 2026-08-11.                                                                                              | Top-level `operands` (JSON string; parse with `json.loads`) + `dd_trace_id` / `dd_span_id` / `dd_env`.                                                                                                     |
| NDCONE (Condor / DE) | `Ndcone[<ACCOUNT>] shopping/flight`                                                                                                       | Raw Condor NDC 21.3 `shopping/flight` call + response for one reprice operand. `<ACCOUNT>` is the pricing account (`NDCONEDECAD`, `NDCONEDEUSD`, `NDCONEDEEUR`). Endpoint `https://api.ndc.condor.com/de/21_3/shopping/flight`. Request is a city-pair shop (`marketingCarrierFlightNumberText=0000`, carrier `DE`), not a pinned 4-segment itinerary. Confirmed 2026-08-11. Condor error `1000` / `NATIVE ParseError [1,1] Content is not allowed in prolog` is **not** 100%: booking `306373201` (2026-08-18) returned `error=[]` and 6 one-way offers. Same JSON envelope as a successful DE-only one-way on 2026-07-30 (`3420ab0fd2aa054a160a53f00080264b`) — schema did not change. | Top-level `Request`, `Response` (JSON strings — **readable, not placeholders**), `data` (`Operation` / `Url` / `Response time`), `backtrace`. Parse `Response` with `json.loads`. Success shape: `error=[]`, `response.offersGroup.carrierOffers[].offer[]` (one-way; `offerItem.service.offerServiceAssociation.paxJourneyRef` + `dataLists.paxJourneyList` / `datedMarketingSegmentList`). `dataLists.paxSegmentList[].marketingCarrierRBDCode` is **unstable**: Condor-operated DE one-way can put carrier `DE` there (2026-07-30 YYZ–FRA DE2403); B6+DE interline puts booking class `Y` there (2026-08-18 MCO–FRA). Builder treats that field as carrier, so DE-only flights match and interline flights become `Y552`. Failure shape: Condor `error[]` + `payloadAttributes.trxID`. |
| NDCONE (Condor / DE) | `Ndcone[<ACCOUNT>] offer/price`                                                                                                           | Condor NDC 21.3 offer-price (availability) call after a shopping match. Fires only when `matchingPackage()` kept at least one package. Present on the 2026-07-30 DE2403 win; absent on the 2026-08-18 interline `No fares found`. Confirmed 2026-08-18. | Top-level `Request`, `Response` (JSON strings), `data`. Success: `error=[]`, `response.pricedOffer` (no `offersGroup`). |
| NDCONE (Condor / DE) | `ndcone Reprice failure`                                                                                                                  | Wrapper when `repriceNdcOne()` throws `IncalculableRepricerException("No fares found")`. Fires both when shopping is empty **and** when Condor returned offers but `PackageCollection::matchingPackage()` dropped every package (itinerary mismatch). Confirmed 2026-08-11 / 2026-08-18. `optimizePackage()` does not pass a debug log into `matchingPackage()`, so `ndcone-no-matching-itineraries` is usually absent. | Top-level `exception`.                                                                                                                                                                                    |
| NDCONE (Condor / DE) | `Reprice <reprice_type>+NDCONEDE{CAD,USD,EUR}+published`                                                                                  | Reprice wrapper for NDCONE (Condor) pricing accounts. Confirmed 2026-08-11.                                                                                                                                                        | Top-level `exception`, `post`, `get`.                                                                                                                                                                     |
| Navitaire-ndc (PD)   | `Reprice <reprice_type>+NAVPD{CAD,USD}+published`                                                                                         | Reprice wrapper for Porter (PD) Navitaire-ndc offices `NAVPDCAD` / `NAVPDUSD`. `reprice_type` ∈ {`original`, `multicurrency`, `new_package`}. Confirmed 2026-08-19 on PD multi-office coverage.                                       | Top-level `exception`, `post`, `get`.                                                                                                                                                                     |
| Navitaire-ndc (PD)   | `Navitaire-NDC-Request-Response-Trace-AirShoppingRQ` / `Navitaire-NDC-Request-Response-Trace-OfferPriceRQ`                                 | Raw Navitaire NDC `AirShopping` / `OfferPrice` call + response for one reprice. Confirmed 2026-08-19.                                                                                                                              | Top-level request / response payload (use the permalink).                                                                                                                                                 |
