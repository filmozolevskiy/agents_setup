...
Loaded from `SKILL.md` Step 2 (harvest) and Step 4 (card template). Specs: [`../docs/2026-08-20-card-dod-qa-design.md`](../docs/2026-08-20-card-dod-qa-design.md) (harvest / short body), [`../docs/2026-08-27-native-checklists-qa-design.md`](../docs/2026-08-27-native-checklists-qa-design.md) (native checklists).

At most two numbered lines before **EXPECT:** (action, then observe). Do not use a Log to open slot. The last observable line is **EXPECT:**. Description has no markdown `- [ ]` and no `✅❌❓`. The native item is the one-line twin under **Native checklists**.
...
```
DoD1 Test 1: Baggage Optimization verify uses the four-part payload — EXPECT later verify routing.payload is V4#uuid#N#longId, not V4#uuid
```
...
```
Fare above the new loss limit is dropped — EXPECT status=0 and error=loss_limit_fare_increase
```
