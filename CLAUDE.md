...
| [`qa_strategy`](.cursor/skills/qa_strategy/SKILL.md) | User wants a QA strategy, test plan, or staging/post-deploy checklist derived from a Trello card and its linked PR — "QA strategy for this card", "staging checklist for <card>", "what should we test for this PR", "post-deploy checks for <card>" | Reads card description via Trello REST API, extracts linked PR URLs, fetches changed files via GitHub MCP, derives a two-section plan (Staging smoke/happy-path/edge-cases/regression risks + Post-deployment checks/monitoring queries/rollback signals); optional `--mode comment` posts plan back to the card |
