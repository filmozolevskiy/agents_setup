...
| [`reporter`](.cursor/skills/reporter/SKILL.md) | Another skill or the user wants a finished message delivered to a specific person or group via Gmail or Slack — "email <name> the result", "slack <channel>", "DM <user>", "notify <team>", "report this to <person>" | Single-purpose delivery skill: caller passes channel (`gmail` / `slack`), literal recipient (email / Slack ID / channel), subject (email only), and a finished markdown body; skill reads the configured Gmail / Slack MCP tool schemas, maps fields, sends once, returns the message ID / `ts`; no drafting, no name resolution, no silent channel fallback |
