...
These rules apply to every session, regardless of which skill is active. Skill-specific discipline (DB CLIs and `db-docs/` policy, query patterns, runner flags, card structure) lives inside each `SKILL.md`. The `table_analysis` skill is the canonical home for DB foundations (CLIs, `.env` loading, `db-docs/` first, durable-fact write-back).
...
| **Evidence-backed claims** | Every factual claim about data, behavior, code, or process is backed by a concrete artefact pasted or linked inline: a query (with slice / window / timezone), a `db-docs/` row, a log permalink, a sample document, a code excerpt with file path, a PR / commit ref, a runner output dump, or a Trello card link. Numbers always state the window, the timezone, and the source CTE or `$match`. Inferences without an artefact are flagged ("looks like…", "I'd expect…") and proposed as the next thing to verify, not stated as fact. Reuse existing rate / count / dedup expressions instead of inventing parallel definitions. |
...
2. **Read the matching skill.** Open `.cursor/skills/<name>/SKILL.md` end-to-end before generating queries, drafting cards, or running runners. Open sibling files (`references/*`, `automation_cards.md`, etc.) only when `SKILL.md` points to them. The skill carries its own discipline (DB CLIs, `db-docs/` policy, CTE patterns, denominator hygiene, runner flags). Any DB-touching task pulls in the `table_analysis` skill's `## DB foundations` section first.
...
|-------|-----------|----------------|
| [`table_analysis`](.cursor/skills/table_analysis/SKILL.md) | Any DB-touching task starts here for foundations; user names a table or collection and wants its purpose / columns / docs; user needs data but no `db-docs/` entry fits ("which table has…", "find table") | DB foundations (CLI scripts, `.env` loading, "no invented connections", `db-docs/` first, durable-fact write-back, ask before guessing the table); two-phase Explore → Document workflow for ClickHouse / MySQL / MongoDB; saves reference docs under `db-docs/<store>/<name>.md` |
