...
| [`code_review`](.cursor/skills/code_review/SKILL.md) | User wants a genesis PR reviewed against the team's coding standards — "review this PR", "code review for <PR>", "what would JP flag on this PR", "check this diff against our standards", "QA my own draft PR"; also "refresh code_review standards" / "re-mine JP comments" for the refresh motion | Loads a mined corpus of Rules (`R<NN>`) at [`references/standards.md`](.cursor/skills/code_review/references/standards.md), each backed by ≥2 of JP's (`jpleveille-mv`) historical PR review permalinks; walks the PR diff via the GitHub MCP; emits findings to chat (rule ID + file:line + Evidence permalink + severity); optional **gated post-back** posts findings as a single GitHub PR review (comments only — never `APPROVE` / `REQUEST_CHANGES`) on explicit `approve` against the full payload shown in chat. Also covers initial mining (corpus dump → histogram gate → draft) and incremental refresh (3-bucket diff gate: New / Reinforced / Going stale). Genesis only |
...
| [`grill_with_docs`](.cursor/skills/grill_with_docs/SKILL.md) | User wants to stress-test a plan against the project's documented language and decisions, says "grill me with docs", or wants terminology sharpened and `CONTEXT.md` / ADRs updated as decisions crystallise | Relentless one-question-at-a-time interview that challenges the plan against the existing domain glossary, cross-references claims with the code, and updates `CONTEXT.md` (glossary, per `references/CONTEXT-FORMAT.md`) inline plus offers ADRs sparingly (`references/ADR-FORMAT.md`). Adapted from `mattpocock/skills` |
...
| [`trello_assistant`](.cursor/skills/trello_assistant/SKILL.md) | Creating or updating cards on the Content Integration or Content Integration - AI Automation boards; weekly grooming / in-flight reports; working a card the user pointed at | Lean SKILL.md router + `references/` (card_anatomy, dedup, grooming, todo_responses); approval gate — every card write (create / update / move / label / member / comment / archive) shown in chat for sign-off before sending; H3 `⊙` card layout — 3 mandatory sections (Short description / Details with inline debug-log permalinks / Visibility with debuggable-CTE or Shape A/B query + count) + 4 optional (Possible solution-expected behavior / Credentials-access for new integrations only / QA notes / Similar-relevant cards); heading-attached content with one blank line between sections; plain ESL-friendly language + `GLOSSARY.md` terms; short titles; AI footer; short human-style intake on AI Automation board; mandatory Filipp-as-member; one card → one branch → one PR lifecycle |
