...
| [`trello_assistant`](.cursor/skills/trello_assistant/SKILL.md) | Creating or updating cards on the Content Integration or Content Integration - AI Automation boards; weekly grooming / in-flight reports; working a card the user pointed at | Lean SKILL.md router + `references/` (card_anatomy, dedup, grooming, todo_responses); card-description area checklist (Summary: what fails / where / why we care; Numbers/Examples: scale or breakdown table / evidence permalinks / debuggable-CTE reproduction / related work / optional one-liners) + AI footer; two-section `⊙` structure on Content Integration board, short human-style intake on AI Automation board; mandatory Filipp-as-member; Shape A/B Mongo pipelines for card-embedded queries; one card → one branch → one PR lifecycle |
