...
| Term | What it means |
|------|---------------|
| **search results page** | The storefront page that lists flight options after a user submits a search. |
| **checkout page** | The storefront page where the user enters passenger / payment details after picking a flight. |
| **check availability call** | The availability check that runs automatically when the user lands on the checkout page. |
...
| **the user** / **the agent** | The end customer using the storefront / the internal FH or JF employee using ResPro. |
